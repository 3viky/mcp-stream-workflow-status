...
function getWorktreeCommits(stream: Stream): Commit[] {
  try {
    ...
    // Get commits from current branch (worktrees are already on their branch)
    // Use HEAD instead of branch name to avoid branch reference issues
    // Limit to recent commits to avoid timeout (last 50 commits or 7 days)
    const gitLogCommand = `git log HEAD --pretty=format:"%H|%an|%aI|%s|" --numstat --since="7 days ago" -n 50`;
